...
export class AuthComponent {
  ...
  ngOnInit(): void {
    ...
    console.log('oninit')
    this.isLoading = true;
    this.functionalityService.getSocketIdWhenReady().then(socketId => {
  ...
  if (this.authForm.valid) {
    this.isLoading = true;
    this.decision = null; // Reinicia decisión anterior
    ...
    try {
      ...
      this.functionalityService.startProcess(data, this.socketId, this.isRetry).subscribe((res) => {
        console.log(res);
      });
      /* this.decisionId = response.decisionId; // Nuevo decisionId
      this.messageId = response.messageId; */  // Nuevo messageId
      this.isRetry = false;                // Reinicia bandera

      // Suscribe a decisiones usando `socketId` como clave
      this.functionalityService.onDecision(this.socketId).subscribe((res) => {
        this.decision = res;

        if (this.decision === 'continue') {
          this.toastr.success('Sesión activa');
          this.router.navigate(['/formulario']);
        } else if (this.decision === 'errorLogin') {
          this.authForm.reset();
          this.toastr.error('Credenciales incorrectas');
          this.isRetry = true; // Activa reintento para próxima llamada
          this.functionalityService.startProcess(data, this.socketId, this.isRetry).subscribe({
            error: (err) => {
              this.isLoading = false;
